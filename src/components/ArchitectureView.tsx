...
function routeEdge(from: { x: number; y: number }, to: { x: number; y: number }): string {
...
function isVisible(nodeX: number, nodeY: number, pan: { x: number; y: number }, zoom: number, canvasW: number, canvasH: number): boolean {
  const BUFFER = 100
  const screenX = nodeX * zoom + pan.x
  const screenY = nodeY * zoom + pan.y
  const sw = NODE_W * zoom
  const sh = NODE_H * zoom
  return screenX + sw > -BUFFER && screenX < canvasW + BUFFER && screenY + sh > -BUFFER && screenY < canvasH + BUFFER
}
...
function computeLayout(nodes: ArchNode[], edges: ArchEdge[]) {
  ...
  const microserviceRects = Array.from(byService.entries()).map(([name, serviceNodes]) => {
    ...
    return {
      ...
      y: Math.min(...ys) - 15,
      width: NODE_W + 20,
      height: Math.max(...ys) + NODE_H + 15 - Math.min(...ys) + 15,
    }
  })
...
export function ArchitectureView(): JSX.Element {
  ...
  const [canvasDims, setCanvasDims] = useState({ w: 1200, h: 600 })
  ...
  const layout = useMemo(() => {
    if (!graph || graph.nodes.length === 0) {
      return { layoutMap: new Map<string, { x: number; y: number }>(), edges: [] as ArchEdge[], microserviceRects: [] as { name: string; x: number; y: number; width: number; height: number }[], layerGroups: [] as { layer: string; x: number; y: number }[], totalW: 600, totalH: 400 }
    }
    return computeLayout(graph.nodes, graph.edges)
  }, [graph])

  const visibleNodeIds = useMemo(() => {
    if (layout.layoutMap.size === 0) return new Set<string>()
    const ids = new Set<string>()
    layout.layoutMap.forEach((pos, id) => {
      if (isVisible(pos.x, pos.y, pan, zoom, canvasDims.w, canvasDims.h)) {
        ids.add(id)
      }
    })
    return ids
  }, [layout.layoutMap, pan, zoom, canvasDims])
  ...
  const handleGenerateTest = useCallback(async (node: ArchNode): Promise<void> => {
    try {
      ...
      const testCode = await invoke<string>('generate_test', { path: node.path, layer: node.layer })
  ...
  if (!graph) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', flexDirection: 'column', gap: 12, color: '#4b5563' }}>
        <div style={{ width: 24, height: 24, border: '2px solid #4f8ef7', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin .7s linear infinite' }} />
        <span style={{ fontSize: 13 }}>Construyendo mapa de arquitectura...</span>
      </div>
    )
  }

  if (layout.layoutMap.size === 0) {
    return (
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', width: '100%', background: '#0d0f14' }}>
      ...
      <div ref={containerRef} style={{ position: 'relative', flex: 1, overflow: 'hidden' }}>
        <svg width="100%" height="100%" onWheel={handleWheel} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp} style={{ cursor: isPanning ? 'grabbing' : 'grab', display: 'block' }}>
          <defs>
          ...
          <g transform={`translate(${pan.x},${pan.y}) scale(${zoom})`}>
            ...
            {layout.microserviceRects.map((ms) => (
              <g key={ms.name}>
                ...
                <text x={ms.x + 10} y={ms.y + 16} fontSize={10} fill="#4b5563" fontWeight={600} style={{ letterSpacing: 1 }}>{ms.name.toUpperCase()}</text>
              </g>
            ))}

            {layout.layerGroups.map((lg, i) => (
              <text key={i} x={lg.x} y={lg.y} fontSize={9} fill="#374151" style={{ letterSpacing: '0.08em', textTransform: 'uppercase' }}>{lg.layer}</text>
            ))}
            ...
            {layout.edges.map((edge) => {
              ...
              const edgeOpacity = !hoveredNode || hoveredNode === edge.from || hoveredNode === edge.to ? 0.6 : 0.15
              return <path key={`${edge.from}-${edge.to}`} d={routeEdge(from, to)} fill="none" stroke={color} strokeWidth={1.2} strokeDasharray={dash} markerEnd="url(#arr)" style={{ opacity: edgeOpacity, transition: 'opacity 0.15s' }} />
            })}

            {visibleNodes.map((node) => {
              const pos = layout.layoutMap.get(node.id)
              if (!pos) return null
              const style = LAYER_STYLES[node.layer] || LAYER_STYLES.UNKNOWN
              const badge = getIssueBadge(node)
              return (
                <g key={node.id} style={{ cursor: 'pointer' }} onClick={() => setSelectedNode(node)} onMouseEnter={() => handleNodeEnter(node.id)} onMouseLeave={handleNodeLeave}>
                  <title>{node.name} — {node.pkg}</title>
                  <rect x={pos.x} y={pos.y} width={NODE_W} height={NODE_H} rx={6} fill={style.bg} stroke={selectedNode?.id === node.id ? style.accent : style.border} strokeWidth={selectedNode?.id === node.id ? 2 : 1} />
                  <rect x={pos.x} y={pos.y} width={4} height={NODE_H} rx={2} fill={style.accent} />
                  <text x={pos.x + 14} y={pos.y + 20} fontSize={12} fontWeight={500} fill={style.textColor}>{truncate(node.name, 22)}</text>
                  <text x={pos.x + 14} y={pos.y + 34} fontSize={10} fill="#4b5563">{style.label}</text>
                  {badge && (
                    <g>
                      <circle cx={pos.x + NODE_W - 10} cy={pos.y + 10} r={9} fill={badge.color} />
                      <text x={pos.x + NODE_W - 10} y={pos.y + 14} fontSize={9} fontWeight={600} textAnchor="middle" fill="#fff">{badge.count > 9 ? '9+' : badge.count}</text>
                    </g>
                  )}
                </g>
              )
        ...
        <div style={{ position: 'absolute', bottom: 14, left: 12, fontSize: 10, color: '#374151' }}>{Math.round(zoom * 100)}%</div>
        ...
        {selectedNode && (
          <div style={{ position: 'absolute', top: 10, right: 10, width: 240, background: '#1a1e28', border: '0.5px solid #2a2f3d', borderRadius: 8, padding: 14, zIndex: 10, fontSize: 12, color: '#9ca3af', boxShadow: '0 8px 24px rgba(0,0,0,0.4)' }}>
            <button onClick={() => setSelectedNode(null)} style={{ position: 'absolute', top: 8, right: 8, background: 'none', border: 'none', color: '#6b7280', cursor: 'pointer', padding: 2, lineHeight: 0 }}><X size={14} /></button>
            <div style={{ marginBottom: 10 }}>
              <h3 style={{ margin: 0, fontWeight: 600, color: '#e5e7eb', fontSize: 14, paddingRight: 20 }}>{selectedNode.name}</h3>
              <span style={{ display: 'inline-block', marginTop: 4, fontSize: 10, padding: '1px 8px', borderRadius: 100, fontWeight: 500, background: LAYER_STYLES[selectedNode.layer]?.bg || '#13161d', color: LAYER_STYLES[selectedNode.layer]?.textColor || '#9ca3af' }}>{selectedNode.layer}</span>
            </div>
              ...
              <div style={{ marginBottom: 10 }}>
                ...
                {selectedNode.endpoints.map((ep, i) => (
                  <div key={i} style={{ fontFamily: 'monospace', fontSize: 10, marginBottom: 2 }}>
                    <span style={{ padding: '0 4px', borderRadius: 2, fontWeight: 600, marginRight: 4, color: ep.method === 'GET' ? '#4ade80' : ep.method === 'POST' ? '#60a5fa' : ep.method === 'PUT' ? '#fbbf24' : '#f87171', background: ep.method === 'GET' ? 'rgba(74,222,128,0.15)' : ep.method === 'POST' ? 'rgba(96,165,250,0.15)' : ep.method === 'PUT' ? 'rgba(251,191,36,0.15)' : 'rgba(248,113,113,0.15)' }}>{ep.method}</span>
              ...
              <div style={{ marginBottom: 10 }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: 6 }}>
                  {(() => {
                    ...
                    return <>{c > 0 && <span style={{ display: 'flex', alignItems: 'center', gap: 2, fontSize: 9, padding: '1px 6px', background: 'rgba(239,68,68,0.25)', color: '#fca5a5', borderRadius: 100 }}><AlertCircle size={9} /> {c} CRITICAL</span>}{w > 0 && <span style={{ display: 'flex', alignItems: 'center', gap: 2, fontSize: 9, padding: '1px 6px', background: 'rgba(245,158,11,0.25)', color: '#fcd34d', borderRadius: 100 }}><AlertTriangle size={9} /> {w} WARNING</span>}{inf > 0 && <span style={{ display: 'flex', alignItems: 'center', gap: 2, fontSize: 9, padding: '1px 6px', background: 'rgba(96,165,250,0.25)', color: '#93c5fd', borderRadius: 100 }}><Info size={9} /> {inf} INFO</span>}</>
                  })()}
                </div>
                <div style={{ maxHeight: 96, overflowY: 'auto' }}>
                  {selectedNode.issues.slice(0, 5).map((iss, i) => (
                    <p key={i} style={{ margin: '0 0 2px 0', fontSize: 10, lineHeight: 1.4 }}><span style={{ fontWeight: 600, marginRight: 4, color: iss.severity === 'CRITICAL' ? '#f87171' : iss.severity === 'WARNING' ? '#fbbf24' : '#60a5fa' }}>•</span>{iss.message}</p>
                  ))}
            ...
            <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
              <button onClick={() => handleViewFile(selectedNode)} style={{ flex: 1, padding: '5px 8px', background: '#4f8ef7', color: '#fff', border: 'none', borderRadius: 4, fontSize: 11, fontWeight: 500, cursor: 'pointer' }}>Ver archivo</button>
              <button onClick={() => handleGenerateTest(selectedNode)} style={{ flex: 1, padding: '5px 8px', background: '#7c6af7', color: '#fff', border: 'none', borderRadius: 4, fontSize: 11, fontWeight: 500, cursor: 'pointer' }}>Generar test</button>
            </div>
          </div>
        )}
      </div>
